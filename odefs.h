 ...
 * huge amounts of disk space and are annoyed at waiting a few more
 * seconds on save and restore.
 */
...
/*
 * If CATCH_SIGNALS is set to 1, will not dump core, nicer for
 * players. dbx still intercepts the signals first, so it's ok for
 * debugging.
 */
...
/* Don't chance anything from here on (unless you know what you're doing) */
#define VERSION 75
...
/*
 * As in attack low, block high, etc.  These values may be added to the ones
 * above to get things like block high, cut low, etc. CLEVER is only used by
 * some monsters to cheat with...
 */
#define LOW 1
#define CENTER 2
#define HIGH 3
...
/* Random aux constants. Aux field for private residences in city */
#define BURGLED 2
#define LOCKED 3
#define UNLOCKED 0

/* Cannot use M command on site with aux value */
...
/* 
 * RETURNING is somewhat deviant -- how many turns until 
 * RETURN spell goes off 
 */
#define RETURNING 24

/*
 * Player immunity indices, also monster immunity bits (2^n), 
 * also damage types 
 */
...
/* Normal room name indices start after the RS_constants */
#define ROOMBASE 39
#define RS_COURT 38
#define RS_ZORCH 37
#define RS_CIRCLE 36
...
/* Objects, location, and terrain; characters to draw */
#define SPACE ' '
#define WALL '#'
#define PORTCULLIS '7'
#define OPEN_DOOR '|'
 ...
 * 
 * It's conceivable for a function of one type to be called when
 * another would usually occur. A monster's special function may be an
 * extra move, for example.
 */
...
/*
 * Some monster ids: (Those that are explicitly named in code)
...
/*
 * City level shope and guild functions.
...
/* LEVITATION_AVOIDANCE, no effect if player is levitating */
 ...
 * Slot 0 should not be filled wen out of inventory_control()
 */
...
struct room {
    ...
    int top;
    int bottom;
    int rsi; /* Index into roonname switch */
};
...
struct player {
    ...
    int birthday;
    char preference;
    int sx; /* Sanctuary coordinates */
    int sy; /* Sanctuary coordinates */
    int x; /* Player coordinates */
    int y; /* Player coordinates */
    int itemweight;
    int maxweight;
...
struct location {
    ...
    uchar lstatus;   /* seen, stopsrun, lit, secret, etc. */
    char roomnumber; /* So room can be named */
    char locchar;    /* Terrain type */
    char showchar;   /* Character instanteously drawn for site */
    int aux;         /* Signifies various things */
    uchar buildaux;  /* Used in constructing levels */
...
struct level {
    ...
    struct monsterlist *mlist; /* List of monsters on this level */
    int environment;           /* What kind of level is this? */
    int deepest;               /* If leve is dungeon head, deepest level made */
};
