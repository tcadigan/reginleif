...
void load_village(int villagenum)
{
    ...
    switch(villagenum) {
    case 1:
        strcat(Str3, "ovillage1.dat");

        break;
    case 2:
        strcat(Str3, "ovillage2.dat");

        break;
    case 3:
        strcat(Str3, "ovillage3.dat");

        break;
    case 4:
        strcat(Str3, "ovillage4.dat");

        break;
    case 5:
        strcat(Str3, "ovillage5.dat");
...
{
    pml tml = (pml)malloc(sizeof(mltype));
    Level->site[i][j].creature = make_creature(SHEEP);
    tml->m = Level->site[i][j].creature;
    tml->m->x = i;
    tml->m->y = j;
    tml->next = Level->mlist;
...
{
    pml tml = (pml)malloc(sizeof(mltype));
    Level->site[i][j].creature = make_creature(HORSE);
    tml->m = Level->site[i][j].creature;
    tml->m->x = i;
    tml->m->y = j;
    tml->next = Level->mlist;
...
{
    pml tml = (pml)malloc(sizeof(mltype));
    Level->site[i][j].creature = make_creature(ML0 + 6);
    tml->m = Level->site[i][j].creature;
    tml->m->x = i;
    tml->m->y = j;
    tml->next = Level->mlist;
...
void assign_village_function(int x, int y, int setup)
{
    ...
    /* Number of x's in village map */
    static int permutation[24];
