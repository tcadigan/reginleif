...
void GetUart(void)
{
    ...
    if (CheckParam("-com2")) {
        comport = 2;
    } else if (CheckParam("-com3")) {
        comport = 3;
    } else if (CheckParam("-com4")) {
        comport = 4;
    } else {
        comport = 1;
    }
...
void InitPort(void) {
    ...
    // Find the irq and IO address fo the port
    GetUart();
    ...
    // Set baud rate
    OUTPUT(uart + LINE_CONTROL_REGSITER, 0x83);
    ...
    if (CheckParam("-8250")) {
        ...
        OUTPUT(uart + FIFO_CONTROL_REGISTER, FCR_FIFOE_ENABLE + FCR_TRIGGER_04);
        tmp = INPUT(uart + INTERRUPT_ID_REGISTER);
    ...
    OUTPUT(uart + INTERRUPT_ENABLE_REGISTER,
           IER_RX_DATA_READY + IER_TX_HOLDING_REGISTER__EMPTY);
...
int read_byte(void)
{
    ...
    x = inque.data[inque.tail & (QUESIZE - 1)];
    ++inque.tail;
...
void interrupt isr_8250(void)
{
    ...
    while (1) {
        ...
        case IIR_MODEM_STATUS_INTERRUPT: // Not enabled
            modem_status = INPUT(uart + MODE_STATUS_REGISTER);

            break;
        case IIR_LINE_STATUS_INTERRUPT: // Not enabled
            line_status = INPUT(uart + LINE_STATUS_REGISTER);

            break;
        case IIR_TX_HOLDING_REGISTER_INTERRUPT: // Transmit
            // I_ColorBlack(63, 0, 0);
            ...
            break;
        case IIR_RX_DATA_READY_INTERRUPT: // Receive
            // I_ColorBlack(0, 63, 0);
