...
/* Update screen when monsters move about    -RAK- */
...
void get_moves(int monptr, int *mm)
{
    int y;
    int ay;
    int x;
...
void make_attack(int monptr)
{
    ...
    if((0x10000 & c_ptr->cmove) && (!py.flags.see_inv)) {
        ...
        sprintf(cdesc, "The %s ", c_ptr->name);
    }
    ...
    while(strlen(attstr) > 0) {
        ...
        if(flag) {
            ...
            case 3:
                ...
                break;
            case 4:
                msg_print(strcat(tmp_str, "strings you."));
            ...
            case 19:
                ...
                break;
            case 99:
                msg_print(strcat(tmp_str, "is repeled."));
            ...
            case 1: /* Normal attack */
                dam = damroll(damstr);
                dam -= (int)((((p_ptr->pac + p_ptr->ptoac) / 200.0) * dam) + 0.5);
                take_hit(dam, ddesc);
                prt_chp();
            ...
            case 4: /* Fear attack */
                ...
                if(player_saves(wis_adj())) {
                    msg_print("You resis the effects!");
                }
            ...
            case 10: /* Blindness attack */
                ...
                if(f_ptr->blind < 1) {
                    ...
                    msg_print("You eyes begin to sting.");
                }
            ...
            case 12: /* Steal money */
                if((randint(124) < py.stats.cdex) && (py.flags.paralysis < 1)) {
                    ...
                    msg_print("You purse feels lighter.");
                    prt_gold();
                }
            ...
            case 13: /* Steal object */
                if((randint(124) < py.stats.cdex) && (py.flags.paralysis < 1)) {
                    ...
                    inven_destroy(i);
                    msg_print("You backpack feels lighter.");
                }
            ...
            case 17: /* Lose intelligence */
                ...
                if(f_ptr->sustain_int) {
                    ...
                    msg_print("You feel you memories fading...");
...
/* Make the move if psosible, five choices    -RAK- */
int make_move(int monptr, int *mm)
{
    int i;
    int j;
    int newy;
                ...
                if(py.flags.confuse_monster) {
                    ...
                    if((randint(MAX_MONS_LEVEL) < r_ptr->level)
                       || (0x1000 & r_ptr->cdefense)) {
                        sprintf(tmp_str, "%s is unaffected.", m_name);
                        msg_print(tmp_str);
                    }
                    else {
                        sprintf(tmp_str, "%s appears confused.", m_name);
                        msg_print(tmp_str);
                        m_ptr->confused = TRUE;
                    }
                }
...
/* Creature can cast spells too. (Dragon breath)    -RAK- */
/* cast_spell = true if creature changes position */
/* took_turn = true if creature casts a spell */
int mon_cast_spell(int monptr, int *took_turn)
{
    unsigned int i;
    int y;
    int x;
    ...
    /* 1 in x change of casting spell */
    if(randint(chance) != 1) {
        cast = FALSE;
        *took_turn = FALSE;
    }
        ...
        *took_turn = TRUE;
        ...
        else if((0x10000 & c_list[m_ptr->mptr].cmove) && !py.flags.see_inv) {
            flag = FALSE;
        }
        else if(py.flags.blind > 0) {
            flag = FALSE;
        ...
        /* Extract all possible spells int spell_choice */
        ...
        case 13: /* Cause fear */
            ...
            if(player_saves(wis_adj())) {
                msg_print("You resist the effect of the spell.");
            }
...
int mon_move(int monptr)
{
    ...
    if(!move_test) {
        /* 75% random movement */
        if((randint(100) < 75) && (c_ptr->cmove & 0x00000020)) {
            mm[0] = randint(9);
            mm[1] = randint(9);
            mm[2] = randint(9);
            mm[3] = randint(9);
        ...
        else if((randint(100) < 40) && (c_ptr->cmove & 0x00000010)) {
            /* 40% random movement */
            mm[0] = randint(9);
            mm[1] = randint(9);
            mm[2] = randint(9);
        ...
        else if((randint(100) < 20) && (c_ptr->cmove & 0x00000008)) {
            mm[0] = randint(9);
            mm[1] = randint(9);
            mm[2] = randint(9);
            mm[3] = randint(9);
...
/* Creature's movement and attacking are done from here    -RAK- */
void creatures(int attack)
{
    ...
    /* Main procesure for creatures    -RAK-*/
    ...
    while((i > 0) && (!moria_flag)) {
        m_ptr = &m_list[i];
        m_ptr->cdis =
            distance(char_row, char_col, (int)m_ptr->fy, (int)m_ptr->fx);
        ...
        if(attack) {
            ...
            if(k > 0) {
                for(j = 0; j < movement_rate(m_ptr->cspeed); ++j) {
                    if((m_ptr->cdis <= c_list[m_ptr->mptr].aaf) || m_ptr->ml) {
                        if(m_ptr->csleep > 0) {
