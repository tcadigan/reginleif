...
int item_value(treasure_type item)
{
	...
	if(index(i_ptr->name, '^') != 0) {
		...
		value = (i_ptr->cost + (i_ptr->tohit + i_ptr->todam + i_ptr->toac) * 100) * i_ptr->number;
	    }
	}
	else {
	    if(i_ptr->toac < 0) {
	...
	if(index(i_ptr->name, '^') != 0) {
		...
		value = (i_ptr->cost + (i_ptr->tohit + i_ptr->todam + i_ptr->toac) * 10) * i_ptr->number;
	    }
	}
    }
    else if((i_ptr->tval == 70)
	    || (i_ptr->tval == 71)
	    || (i_ptr->tval == 75)
	    || (i_ptr->tval == 76)
	    || (i_ptr->tval == 80)) {
...
/* Add the item in INVEN_MAX to store's inventory    -RAK- */
...
/*
 * Destroy an item in the store's inventory. Note that if "one_of is false, an
 * entire slot is destroyed -RAK-
 */
...
/* Initializes the store's with owners    -RAK- */
void store_init()
{
    int i;
    int j;
...
void store_create(int store_num)
{
    int i;
    int tries;
    int cur_pos;
    ...
    /* See the store_choice in variables.c */
