...
typedef struct {
    short gameid; // So mlutiple games can setup at once
    short drone;
    short nodesfound;
    short nodeswanted;
} setupdata_t;
...
// Time is used by the communication driver to sequence packets returned to
// DOOM when more than one is waiting
...
extern long localtime; // For timestamp in packet
