...
void LookForNodes(void)
{
    ...
    // Wait until we get [numnetnodes] packets, then start playing the
    // playernumbers are assigned by netid
    ...
    nodesetup[0].nodeswanter = numnetnodes;
    doomcom.numbers = 1;
    ...
    while (1) {
        ...
        while (GetPacket()) {
            ...
            if (doomcom.remotenode != -1) {
                // Already know that node address
                continue;
            }
...
#define MAXARGVS 100
    ...
    for (i = 1; i < myargc; ++i) {
        if (myargv[i][0] == '@') {
            ...
            // Keep all cmdline args following @responsefile argc
...
void main(void)
{
    ...
    if ((i = CheckParm("-vector")) != 0) {
        doomcom.intnum = sscanf("0x%0x", myargv[i + 1]);
