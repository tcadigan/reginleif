/*
 * Omega copyright (C) by Laurance Raphael Brothers, 1987-1989
 *
 * omove.c
 *
...
void l_water()
{
    if(!gamestatusp(MOUNTED)) {
        if(Player.possessions[O_ARMOR] != NULL) {
            print1("Your heavy water drags you under the water!");
            p_drown();
        }
...
void l_chaos()
{
    ...
    if(Player.rank[ADEPT]) {
        if(!onewithchaos) {
            onewithchaos = 1;
            print2("You achieve oneness with Chaos...");
        }
...
void l_abyss()
{
    ...
    if(Current_Environment != Current_Dungeon) {
        ...
        if(random_range(100) == 13) {
            morewait();
            print1("As you fall you see before you want seems like");
...
void l_lift()
{
    ...
    if(response != ESCAPE) {
        ...
        if(response == 'd') {
            change_level(Level->depth, Level->depth + levelnum, FALSE);
        }
        else {
            change_level(Level->depth, Level->depth - levelnum, FALSE);
        }
...
void l_magic_pool()
{
    ...
    print1("This pools seems to be enchanted...");
    ...
    if(gamestatusp(MOUNTED)) {
        ...
        if(random_range(1000) < (Player.level * Player.level * Player.level)) {
            print2("The DREADED AQUAE MORTIS throttles your within inches...");
...
void l_water_station()
{
    ...
    if(ynq1() == 'y') {
        ...
        morewait();
        print2("You body begins to disintegrate...Leave the pool? [yn] ");
...
void l_air_station()
{
    ...
    if(ynq1() == 'y') {
        ...
        dataprint();
        print1("You are buffted and burnt by the storm...");
        print2("You begin to lost consciousness...Leave the storm? [yn] ");
...
void stationcheck()
{
    ...
    if(!stationsleft) {
        ...
        print2("You spin aorund, and don't see anyone around at all");
        print3("except for a spurred black cloacked figure carrying a scythe.");
        morewait();
        clearmsg();
...
/*
 * To survive the void, the other four stations must be visited first, to
 * activate the void, then something (Death's scythe, possibly) must be thrown
 * in to satiate the void, then all other items must be dropped, then the void
 * must be entered.
 */
void l_void_station()
{
...
void l_voice3()
{
    print3("An eerie voice resounds: The void is the fifth Elemental Station.");
    Level->site[Player.x][Player.y].p_locf = L_NO_OP;
}
...
void l_whirlwind()
{
    ...
    if(random_range(2)) {
        ...
        if(random_range(20) == 17) {
            print2("\'I don't thinkg we're in Kansas anymore, toto.\'");
        }
...
void l_tome1()
{
    ...
    menuprint("\nfor either Law of Chaos. It is magically linked to Star Peak");
    menuprint("\nand can either be activated or destroyed there. If destroyed,");
    menuprint("\nits power will be used for Chaos, if activated, for Law.");
    menuprint("\nIt is said the LawBringer has waited an eternity");
    menuprint("\nat Star Peak for someone to bring him the gem.");
    menuprint("\nIt is also rumored that while anyone whight destroy the gem,");
...
void l_tome2()
{
    ...
    menuprint("\nbe allowed to recharge for a long time between uses.");
...
void l_throne()
{
    pob o;
    int i;
    print1("You have come upon a huge orante appointe throne!");
    print2("Sit in it? [yn] ");

    if(ynq1() == 'y') {
        if(!find_item(&o, ARTIFACTID + 22, -1)) {
            print1("The throne emits an eerie voilet-black radiance.");
            ...
            morewait();
            clearmsg();
            print1("The radience finally ceases. You can get up now.");
        }
        else {
            if(HiMagicUse == Date) {
                ...
                case 17:
                    ...
                    if(find_and_remove_item(ARTIFACTID + 22, -1)) {
                        ...
                        print2("it explodes in a spray of shards.");
                    }
    ...
    if(ynq1() == 'y') {
        if(!gamestatusp(COMPLETED_CASTLE)) {
            if(!gamestatusp(ATTACKED_ORACLE)) {
                print2("A dulcet void says: \'Jolly good show!\'");
            }
...
void l_chaostone()
{
    print1("This is a menheir carved of black marble with veins of gold.");
    print2("It emenates an aura of raw chaos, which is not terribly");
    morewait();
    ...
    if(Player.alignment < 0) {
        ...
        print2("You find it extremely difficulty to approach the stone.");
    }
...
void l_balancestone()
{
    print1("This is a msasive grantie slab teetering dangerously on a corner.");
    ...
    if(ynq1() == 'y') {
        ...
        if(abs(Player.alignment) > random_range(50)) {
            ...
            if(ynq2() == 'y') {
                clearmsg();
                print1("The vortex calms down, diminishes, and then disappears.");
            }
            else {
...
void l_lawstone()
{
    ...
    print2("You feel an aura of serenity rising from it, and you gaze");
    morewait();
...
void l_sacrificestone()
{
    ...
    if(ynq2() == 'y') {
        ...
        if(ynq2() == 'y') {
            ...
            print1("You leave some skin behind, though...");
            morewait();
            ...
            if((Player.maxhp > 10) && (Player.maxhp < ((3 * oldmaxhp) / 4))) {
                ...
                print1("You have a sense of rejection.");
...
void l_mindstone()
{
    ...
    if(ynq2() == 'y') {
        ...
        if(stonecheck(0)) {
            print1("You mind has been enhanced by the experience!");
        ...
        print2("The light flahses from the crystal diminish in frequency.");
    }
}
