...
{
    /*
     * Location functions above traps should be activated whether levitating or
     * not
     */
    if(movef != L_NO_OP) {
        clearmsg();
    ...
    /* '@' command activates all effects under player */
    if(!Player.status[LEVITATING]
       || gamestatusp(MOUNTED)
       || (Cmd == '@')
       || (movef < LEVITATION_AVOIDANCE)) {
        switch(movef) {
        case L_NO_OP: /* Miscellaneous */
            l_no_op();
        ...
        case L_MINDSTONE:
            ...
            break;
        case L_ADEPT: /* Challenge functions */
            l_adept();
...
{
    /* 
     * loc functions above traps should be activated whether levitating 
     * or not 
     */
...
void sign_print(int x, int y, int sign)
{
    ...
    case L_MAZE:
        print1("You notice a sign:");
        print2("Hedge closed for trifid extermination.");
    ...
    case L_DINER:
        if(sign) {
            print1("You notice a sign:");
            print2("The Rampart Diner. All you can eat, 25 Au.");
        }
    ...
    case L_CEMETARY:
        print1("You notice a sign:");
        print2("Rampart city Cemetary. Close -- Full.");
