...
int randes_seed;               /* For restarting town_seed */
char town_state[STATE_SIZE];   /* Seed for town generation */
int town_seed;                 /* For restarting town_seed */
int cur_height;                /* Current dungeon size */
int cur_width;                 /* Current dungeon size */
int dun_level = 0;             /* Current dungeon level */
int missile_ctr = 0;           /* Counter for missiles */
int msg_flag;                  /* Set with first message */
...
int turn = 0;                  /* Current turn of game */
...
/*
 * Values for floor types. If fval is more than 4 bits then the save file
 * routines must be changed.
 */
...
int player_exp[MAX_PLAYER_LEVEL] = {
    ...
    150000, 200000, 300000, 400000, 500000, 750000, 15000000, 2500000, 5000000, 10000000
};
...
/*
 * Race: STR, INT, WIS, DEX, CON, CHAR, Ages, Heights, Weights
 * Racial Bases for: dis, srh, stl, fos, bth, bthb, bsav, hitdie, infra, 
 *                   choice-classes
 */
race_type race[MAX_RACES] = {
    { "Human"     ,  0,  0,  0,  0,  0,  0, 14,  6, 72,  6, 180, 25, 66, 4, 120, 20, 1.00,  0,  0,  0,  0,   0,   0,  0, 10, 0, 0x3F },
    { "Half-Elf"  , -1,  1,  0,  1, -1,  1, 24, 16, 66,  6, 130, 15, 62, 6, 100, 10, 1.10,  2,  6,  1, -1,  -1,   5,  3,  9, 0, 0x3F },
    { "Elf"       , -1,  2,  1,  1, -2,  1, 75, 75, 60,  4, 100,  6, 54, 4,  80,  6, 1.20,  5,  8,  1, -2,  -5,  15,  6,  8, 0, 0x1F },
    { "Halfling"  , -2,  2,  1,  3,  1,  1, 21, 12, 36,  3,  60,  3, 33, 3,  50,  3, 1.10, 15, 12,  4, -5, -10,  20, 18,  6, 4, 0x0B },
    { "Gnome"     , -1,  2,  0,  2,  1, -2, 50, 40, 42,  3,  90,  6, 39, 3,  75,  3, 1.25, 10,  6,  3, -3,  -8,  12, 12,  7, 3, 0x0F },
    { "Dwarf"     ,  2, -3,  1, -2,  2, -3, 35, 15, 48,  3, 150, 10, 46, 3, 120, 10, 1.20,  2,  7, -1,  0,  15,   0,  9,  9, 5, 0x05 },
    { "Half-Orc"  ,  2, -1,  0,  0,  1, -4, 11,  4, 66,  1, 150,  5, 62, 1, 120,  5, 1.10, -3,  0, -1,  3,  12,  -5, -3, 10, 3, 0x0D },
    { "Half-Troll",  4, -4, -2, -4,  3, -6, 20, 10, 96, 10, 300, 50, 84, 8, 260, 40, 1.20, -5, -1, -2,  5,  20, -10, -8, 12, 3, 0x05 }
};
...
background_type background[MAX_BACKGROUND] = {
    ...
    { "You have blue-grey eyes, "                         , 100, 50, 51,   0 },
    { "straight "                                         ,  70, 51, 52,   0 },
    { "wavy "                                             ,  90, 51, 52,   0 },
    { "curly "                                            , 100, 51, 52,   0 },
    ...
    { "and a very fari complexion."                       , 100, 53, -1,   0 },
    ...
    { "You have glowing red eyes, "                       , 100, 57, 58,   0 },
    { "straight "                                         ,  90, 58, 59,   0 },
    { "wavy "                                             , 100, 58, 59,   0 },
    { "back hair, "                                       ,  75, 59, 60,   0 },
    { "brown hair, "                                      , 100, 59, 60,   0 },
    { "a one-foot beard, "                                ,  25, 60, 61,   0 },
    { "a two-foot beard, "                                ,  60, 60, 61,   0 },
    { "a three-foot beard, "                              ,  90, 60, 61,   0 },
    { "a four-foot beard, "                               , 100, 60, 61,   0 },
    ...
    { "you have glowing red eyes, "                       , 100, 62, 63,   0 },
    { "dirty "                                            ,  33, 63, 64,   0 },
    { "mangy "                                            ,  66, 63, 64,   0 },
    { "oily "                                             , 100, 63, 64,   0 },
...
double rgold_adj[MAX_RACES][MAX_RACES] = {
    ...
    /* Gonme      */ { 0.15, 0.15, 0.10,  0.05, -0.05,  0.10, 0.15, 0.30 },
    /* Dwarf      */ { 0.15, 0.20, 0.20,  0.10,  0.10, -0.05, 0.25, 0.35 },
    /* Half-Orc   */ { 0.15, 0.20, 0.25,  0.15,  0.15,  0.30, 0.10, 0.15 },
    /* Half-Troll */ { 0.10, 0.15, 0.15,  0.10,  0.10,  0.10, 0.10, 0.10 }
};
...
/*
 * Each character class has a list of spells they an use. In cases the entire
 * array is blank
 */
spell_type magic_spell[MAX_CLASS][31] = {
	...
	{ "Polymorph Object"       , 11,  7, 100, 45, FALSE },
...
treasure_type mush = { "& pint~ of fine grade mush", 80, ',', 0x00000000, 1500, 0, 306, 1, 1, 0, 0, 0, 0, "1d1", 1 };

/*
 * Each type of character starts out with a few provisions...
 * Note the entries refer to array elements of INVENTORY_INIT array
 */
byteint player_init[MAX_CLASS][5] = {
    /* Warrior */ { 0, 103, 41,  6, 32 },
    /* Mage    */ { 0, 103, 41,  6, 66 },
    /* Priest  */ { 0, 103, 41 , 6, 70 },
    /* Rogue   */ { 0, 103,  6, 32, 66 },
    /* Ranger  */ { 0, 103, 41,  6, 66 },
    /* Paladin */ { 0, 103, 41,  6, 70 }
};
...
owner_type owners[MAX_OWNERS] = {
    { "Erik the Honest          (Human)         General Store",   250, 0.75, 0.08, 0.04, 0, 12 },
    ...
    { "Andy the Friendly        (Halfling)      General Store",   200, 0.70, 0.08, 0.05, 3, 15 },
    { "Darg-Low the Grim        (Human)         Armory"       , 10000, 0.90, 0.11, 0.04, 0,  9 },
    { "Oglign Dragon-Slayer     (Dwarf)         Armory"       , 32000, 0.95, 0.12, 0.04, 5,  8 },
...
treasure_type store_door[MAX_STORES] = {
    { "the entrance to the General Store", 110, '1', 0x00000000, 0, 0, 101, 0, 1, 0, 0, 0, 0, "0d0", 0 },
    { "the entrance to the Armory"       , 110, '2', 0x00000000, 0, 0, 102, 0, 1, 0, 0, 0, 0, "0d0", 0 },
    { "the entrance to the Weapon Smiths", 110, '3', 0x00000000, 0, 0, 103, 0, 1, 0, 0, 0, 0, "0d0", 0 },
    { "the entrance to the Temple"       , 110, '4', 0x00000000, 0, 0, 104, 0, 1, 0, 0, 0, 0, "0d0", 0 },
    { "the entrance to the Alchemy Shop" , 110, '5', 0x00000000, 0, 0, 105, 0, 1, 0, 0, 0, 0, "0d0", 0 },
    { "the entrance to the Magic Shop"   , 110, '6', 0x00000000, 0, 0, 106, 0, 1, 0, 0, 0, 0, "0d0", 0 }
};
...
int store_chioce[MAX_STORES][STORE_CHOICES] = {
    ...
    /* Temple           */ {  71,  72,  73,  74,  59,  59, 77,  79,  80, 81, 84, 85, 13, 14, 15, 96, 97, 98, 100, 79, 79, 80,  80, 81, 72, 98 },
    /* Alchemy Shop     */ {  55,  56,  57,  58,  58,  60, 61,  62,  63, 64, 65, 66, 75, 76, 78, 82, 83, 60,  61, 62, 63, 64,  98, 99, 98, 98 },
    /* Magic-User Store */ {  67,  49,  50,  51,  52,  53, 54,  48,  68, 69, 69, 70, 86, 87, 88, 89, 90, 91,  92, 93, 94, 95, 86, 101, 68, 88 }
};
