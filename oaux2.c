 ...
 * oaux2.c
 *
 * Some functions called by ocom.c, also see oaux1.c and oaux3.c. This
 * is a real grab bag file. It contains functions used by oaux1.c and
 ...
 * abilities, chances to do things, etc. Positive is good, negative is
 * bad.
 */
...
/* 
 * Handle a h,j,k,l, etc. to change x and y by dc and dy for targetting in
 * dungeon
 */
...
/*
 * Deal with each possible status -- values are per move. This function is
 * executed every move. A value over 1000 indicates a permanent effect
 */
...
/*
 * Values are in multiples of ten minutes. Values over 1000 indicate a permanent
 * effect
 */
...
/* Kill off player if he hasn't got the "breathing" status */
void p_drown()
{
    if(Player.status[BREATHING] > 0) {
...
/* The effect of some weapon on a monster m, with dmgmod a bonus to damage */
...
/*
 * This function is used to undo all items temporarily, should always be used in
 * pairs with on being TRUE and FALSE, and may cause anomalous stats and
 * item-usage if used indescriminately.
 */
...
/* Switched context dungeon/countryside/city, etc. */
