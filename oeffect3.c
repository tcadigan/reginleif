...
/* Uncurse all items, cure disease, and neutralize poison */
    ...
    if(blessing < 0) {
        ...
        if(target != NULL) {
            ...
            if(!m_immunityp(target, SLEEP)) {
                ...
                strcat(Str1, " is right eyed, and bushy tailed!");
            }
...
void disintegrate(int x, int y)
{
    ...
    if(!inbounds(x, y)) {
        ...
        if(Player.possessions[O_CLOAK] != NULL) {
            ...
            mprint("You armor disintegrates!");
        ...
        if(target != NULL) {
            ...
            mprint("Zzzap! The altar seems unaffected...");
            ...
            if(Level->site[x][y].p_locf == L_TRIFID) {
                mprint("The trifid screams as it disintegrates!");
                gain_experience(50);
                Level->site[x][y].p_locf = L_NO_OP;
                Level->site[x][y].locchar = FLOOR;
                lreset(x, y, SECRET);
...
void apport(int blessing)
{
    ...
    if(blessing > -1) {
        ...
        if(Level->site[x][y].things != NULL) {
            ...
            mprint("There's nothing there to apport1");
        }
    }
    else {
...
void strategic_teleport(int blessing)
{
    ...
    if(blessing < 0) {
        ...
        menuprint("d: Village or Stomrwatch\n");
...
void hero(int blessing)
{
    if(blessing > -1) {
        ...
        level_drain(abs(blessing), "a portion of cowardice");
    }
}
...
/* Has effect of switching between first level and deepest level attained */
void level_return()
{
    if(Current_Environment == Current_Dungeon) {
        if(Level->depth > 1) {
...
void disease(int amount)
{
    ...
    if(!Player.immunity[INFECTION]) {
        mprint("You being to shiver with ague.");
...
void polymorph(int blessing)
{
    ...
    if((x == Player.x) && (y == Player.y)) {
        ...
        else if(m_immunityp(m, OTHER_MAGIC) || (m->level > random_range(12))) {
            strcpy(Str1, "The ");
            ...
            if(blessing < 0) {
                ...
                if(m->uniqueness == COMMON) {
                    ...
                    if(m->possessions != NULL) {
                        /*
                         * Note this does not free the possessions, but onlyter
                         * the head of the object list of poessessions. Not
                         * serious, just a waste of memory.
                         */
                        free((char *)m->possessions);
                    }
                    /* It happens that the transformid is -1 */
                    m->possessions = NULL;
                }
                else {
    ...
    if((x == Player.x) && (y == Player.y)) {
        ...
        if(m != NULL) {
            if((blessing > -1) && !m_immunityp(m, NEGENERGY)) {
                ...
                mprint("the effect reverse itself!");
            ...
            mprint("The altar collapse in on itself...");
...
void inflict_fear(int x, int y)
{
    ...
    if((Player.x == x) && (Player.y == y)) {
        ...
        if(m != NULL) {
            ...
            mprint("A thrill of fear tickels your spine... and passes.");
        }
    }
}
