...
/*
 * Looks for level tolevel in current dungeon which is named by Dungeon, which
 * may be NULL. If the level is found, and rewrite_level is FALSE, and the level
 * has already been generated, nothing happens beyond Level being set
 * correctly. Otherwise the level is created from scratch.
 */
void change_level(char fromlevel, char tolevel, char rewrite_level)
{
    ...
    /* Synchronize with plaer on level change */
...
char *roomname(int index)
{
    ...
    case RS_ODIN:
        ...
        break;
    case RS_ADEPT:
        strcpy(Str4, "Labryinth of the Adept's Challenge.");
    ...
    case RS_WYRM:
        ...
        break;
    case RS_OCEAN:
        strcpy(Str4, "The Undergroung Ocean.");

        break;
    case RS_PONDS:
        strcpy(Str4, "A series of subterraneon pools and streams.");
    ...
    case ROOMBASE + 28:
        ...
        break;
    case ROOMBASE + 29:
        strcpy(Str4, "A chamber with a blue crystal omega dias");
...
/*
 * Puts the player on the first set of stairs form the apt level if can't find
 * them, just drops the player anywhere...
 */
...
/*
 * x,y is top left corner, l is length of side, rsi is room string index, baux
 * is so all rooms will have a key field.
 */
...
void cavern_level()
{
    ...
    if(Current_Dungeon == E_CAVES) {
        if((Level->depth == CAVELEVELS) && !gamestatusp(COMPLETED_CAVES)) {
            ...
            /* Goblin king */
            Level->site[tx][ty].creature = (pmt)make_creature(ML3 + 5);
