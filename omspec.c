...
void m_sp_ng(struct monster *m)
{
    if(distance(m->x, m->y, Player.x, Player.y) < 2) {
        if((random_range(5) == 1) || (Player.status[VULNERABLE] > 0)) {
            ...
            mprint("Its leathery wings flap and flap, and it giggle insanely.");
            mprint("It tickles you cunnignly to render you incapable of escape.");
...
void m_sp_demon(struct monster *m)
{
    ...
    if((m->hp < (m->level * 5)) && (m->hp > 1)) {
        ...
        case 6: /* Frost demon */
            ...
            break;
        case 7: /* Outer circle demon */
            mid = ML5 + 12;

            break;
        case 8: /* Demon serpent */
            mid = ML6 + 10;
...
/*
 * Monsters with this have some way to hide, camoflage, etc. until 
 * they attack
 */
    ...
    if(m_statusp(m, HOSTILE)) {
        if(distance(m->x, m->y, Player.x, Player.y) < 2) {
            if(!Player.status[IMMOBILE]) {
                mprint("A gust of wind from the Dragonlord's wings knows you down!");
    ...
    if(Player.status[IMMOBILE]
       && (distance(Player.x, Player.y, m->x, m->y) < 2)) {
        if(!Player.status[AFRAID]) {
            ...
            mprint("The bogthing's touch causes you to scream in agony!");
...
void m_sp_leash(struct monster *m)
{
    if(m->aux1 < 0) {
        m->aux1 = m->x;
        m->aux2 = m->y;
...
void m_sp_angel(struct monster *m)
{
    ...
    case ATHENA:
    case ODIN:
        if((Player.patron == HECATE) || (Player.patron == SET)) {
            hostile = 1;
        }
        else {
            hostile = 0;
            ...
            if(inbounds(x, y)) {
                if(Level->site[x][y].things != NULL) {
                    if(Level->site[x][y].things->thing->id == CORPSEID) {
                        mprint("The Zombie Overlord makse a mystical gesture...");
        ...
        if(distance(Player.x, Player.y, m->x, m->y) < 2) {
            if(p_immune(THEFT) || (Player.level > ((m->level * 2) + random_range(20)))) {
                ...
                if(i == ABORT) {
                    ...
                    strcat(Str2, "suddenly runs away from some reason.");
                    mprint(Str2);
                    m_teleport(m);
...
void m_sp_merchant(struct monster *m)
{
    ...
    if(m_statusp(m, HOSTILE)) {
        if(Current_Environment == E_VILLAGE) {
            mprint("The merchange screams: \'Help! Murder! Guards! Help!\'");
...
/*
 * The special function of the various people in the court of the
 * archmage and the sorcerors' circle
 */
...
void m_sp_lair(struct monster *m)
{
    ...
    if(m_statusp(m, HOSTILE)) {
        if(m->id == (ML10 + 3)) {
            m->specialf = M_SP_DRAGONLORD;
        }
        
        mprint("You notice a number of dragons waking up...");
        mprint("You are struct by a quantity of firebolts.");
