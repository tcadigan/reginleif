...
void bless(int blessing)
{
    ...
    if(blessing < 0) {
        ...
        if(index == CASHVALUE) {
            ...
            if(Player.possessions[index]->blessing < -(blessing + 1)) {
                ...
                print2("The hieroflux fades without any appreciable effect...");
                morewait();
            }
...
void bolt(int fx, int fy, int tx, int ty, int hit, int dmg, int dtype)
{
    ...
    if((xx == Player.x) && (yy == Player.y)) {
    ...
    else if(Level->site[xx][yy].creature != NULL) {
        ...
        if(Level->site[xx][yy].p_locf == L_HEDGE) {
            ...
            mprint("The trifid absorbs the enegry and laughs!");
        }
    }
...
void ball(int fx, int fy, int tx, int ty, int dmg, int dtype)
{
    ...
    for(i = 0; i < 9; ++i) {
        ...
        if((ex == Player.x) && (ey == Player.y)) {
        ...
        else if(Level->site[ex][ey].creature != NULL) {
            target = Level->site[ex][ey].creature;
            ...
            if(los_p(Player.x, Player.y, target->x, target->y)) {
                ...
                case FLAME:
                    ...
                    break;
                case COLD:
                    strcat(Str1, " was basted by a snowball!");
    ...
    for(ml = Level->mlist; ml != NULL; ml = ml->next) {
        if(ml->m->hp > 0) {
            if(blessing > -1) {
                ...
                plotmon(&Monsters[random_range(NUMMONSTERS)]);
            }
        }
    }
...
int random_item()
{
    ...
    for(tries = 0; (tries < MAXITEMS) && !done; ++tries) {
        ...
        if(Player.possessions[item] != NULL) {
            done = 1;
        }
        else {
            done = 0;
