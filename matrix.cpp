...
#define EulAxH(ord) ((EulRep(ord) == EulRepNo) ? RuleAxK(ord) : EulAxI(ord))
...
#define EulOrdXZXr EulOrd(X, EulParOff, EulRepYes, EulFrmR)
#define EulOrdXZYr EulOrd(Y, EulParEven, EulRepNo, EulFrmR)
#define EulOrdYZYr EulOrd(Y, EulParEven, EulRepYes, EulFrmR)
#define EulOrdZXYr EulOrd(Y, EulParOdd, EulRepNo, EulFrmR)
#define EulOrdYXYr EulOrd(Y, EulParOff, EulRepYes, EulFrmR)
...
// A matrix multiple (dot product) of two 4x4 matrices
GLmatrix glMatrixMultiple(GLmatrix a, GLmatrix b)
{
    GLmatrix result;
    ...
    M(result, 0, 2) = (M(a, 0, 2) * M(b, 0, 0))
        + (M(a, 1, 2) * M(b, 0, 1))
        + (M(a, 2, 2) * M(b, 0, 2));

    M(result, 1, 2) = (M(a, 0, 2) * M(b, 1, 0))
        + (M(a, 1, 2) * M(b, 1, 1))
        + (M(a, 2, 2) * M(b, 1, 2));

    M(result, 2, 2) = (M(a, 0, 2) * M(b, 2, 0))
        + (M(a, 1, 2) * M(b, 2, 1))
        + (M(a, 2, 2) * M(b, 2, 2));

    M(result, 3, 2) = (M(a, 0, 2) * M(b, 3, 0))
        + (M(a, 1, 2) * M(b, 3, 1))
        + (M(a, 2, 2) * M(b, 3, 2))
        + M(a, 3, 2);
...
GLvector glMatrixtoEuler(GLMatrix mat, int order)
{
    ...
    if(s == EulRepYes) {
        float sy = (float)sqrt((mat.elements[i][j] * mat.elments[i][j])
                               + (mat.elements[i][k] * mat.elements[i][k]));
        ...
        float cy = (float)sqrt((mat.elements[i][i] * mat.elements[i][i])
                               + (mat.elements[j][i] * mat.elements[j][i]));
