...
void ident_char()
{
    ...
    if(get_com("Enter chracter to be identified: ", &command)) {
	switch(command) {
	case ' ':
	    prt("  - An open pit.", 0, 0);
...
void original_help();
{
    ...
    prt("D <Dir> Disarm a trap/chest.    | s         Search for hidden doors.", 2, 0);
    ...
    prt("b       Browse a book.          | ^P        Repeat the last message.", 11, 0);
    prt("c <Dir> Close a door.           | ^R        Redraw the screen.", 12, 0);
    prt("d       Drop an item.           | ^K        Quit the game.", 13, 0);
    prt("e       Equipment list.         | ^X        Save character and quit.", 14, 0);
    ...
    prt("j <Dir> Jam a door with spike.  | >         Do down a down-staircase.", 18, 0);
    prt("l <Dir> Look given direction.   | . <Dir>   Move in direction.", 19, 0);
    prt("m       Cast a magic spell.     | Movement: 7  8  9", 20, 0);
    prt("o <Dir> Open a door/chest.      |           4     6    5 == Rest", 21, 0);
...
void rogue_like_help()
{
    clear_screen(0, 0);
    prt("f <Dir>    Base (object/creature). | q           Quaff a potion.", 0, 0);
    prt("C          Display character.      | r           Read a scroll.", 1, 0);
    prt("C <Dir>    Disarm a trap/chest.    | s           Search for hidden doors.", 2, 0);
    prt("E          Eat some food.          | T           Take off an item.", 3, 0);
    ...
    prt("z          Aim and fire a wand.    |", 10, 10);
    prt("p          Browse a book.          | ^P          Repeat the last message.", 11, 0);
    prt("c <Dir>    Close a door.           | ^R          Redraw the screen.", 12, 0);
    ...
    prt("x <Dir>    Look a given direction. | SHIFT <Dir> Move in direction.", 19, 0);
    prt("m          Cast a magic spell.     | Movement:   y  k  u", 20, 0);
    prt("o <Dir>    Open a door/chest.      |             h     l    . == Rest", 21, 0);
...
void original_wizard_help()
{
	...
	prt("^N - Print monster directory.", 11, 0);
