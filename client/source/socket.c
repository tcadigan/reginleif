...
void procesS_gb(char *s)
{
    ...
    check4_endpoint(s); /* Dynamic call here */
...
void process_socket(char *s)
{
    ...
    if(type != NONE) {
        ...
        if(ptr) {
            if(type == GB_EMOTE) {
                ...
                p = str + strlen(s) - strlen(actualmsg) - 2;
            }
    ...
    if(type != NONE) {
        process_spoken(racenme, govname, atoi(raceid), atoi(govid), actualmsg);
    }
    ...
    *dmbuf2 = '\0';
    ...
    if((sscanf(s, "Logged in as %[^:]:%[^[][%d.%d].", dmbuf, dmbuf2, &dmint, &dmint2) == 4)
       && (client_stats == L_PASSWORD)) {
        ...
        sprintf(dmbuf, "%s", profile.raceid);
        add_assign("raceid", dmbuf);
        sprintf(dmbuf, "%s", profile.govid);
    ...
    if((*(s + 1) == '(') && (*(s + 3) == '[')) {
        ...
        *scope.mothership = '\0';
        ...
        if(strchr(s, '#')) {
            ...
            else if((sscanf(s, " ( [%d] /%[^/]/%[^/]/ /../#%[^/]#%s )", &scope.aps, scope.star, scope.plaent, scope.mothership, scope.shipc) == 5)
                    || (sscanf(s, " ( [%d] /%[^/]/%[^/]/#%[^/]/#%s )", &scope.aps, scope.star, scope.planet, scope.mothership, scope.shipc) == 5)) {
                scope.level = LEVEL_PSSHIP;
            }
            else if((sscanf(s, " ( [%d] /%[^/]/ /../#%[^/]/#%s )", &scope.aps, scope.star, scope.mothership, scope.shipc) == 4)
            ...
            else if(sscanf(s, " ( [%d] /%[^/]/#%s )", &scope.aps, scope.star, scope.shipc) == 3) {
                scope.level = LEVEL_SSHIP;
            }
            else if(sscanf(s, " ( [%d] /#%s )", &scope.aps, scope.shipc) == 2) {
...
char *build_scope(void)
{
    ...
    if(scope.level >= LEVEL_SHIP) {
        ...
        sprintf(scopebugf, "/%s/%s", scope.star, scope.planet);
    }
