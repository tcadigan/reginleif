...
void peruse()
{
    ...
    if(Player.status[BLINDED] > 0) {
        print3("You're blind -- you can't read that!!!");
    }
        ...
        if(!inbounds(x, y)) {
            ...
            if(random_range(50 + (difficulty() * 5)) < ((Player.dex * 2) + (Player.level * 3) + (Player.rand[THIEVES] * 10))) {
                ...
                if(random_range(100) < (Player.dex + (Player.rank[THIEVES] * 10))) {
                    ...
                    if(o != NULL) {
                        print2("You manager to retrieve the trap components!");
...
void give()
{
    ...
    if(dindex == ABORT) {
        ...
        if(!inbounds(Player.x + dx, Player.y + dy)) {
            ...
            print3("There nothing there to give something to!!!");
            setgamestatus(SKIP_MONSTERS);
        }
        else {
            m = Level->site[Player.x + dx][Player.y + dy].creature;
...
/* 
 * Set various player options. Have to redefine in odefs for next full recompile
 */
void setoptions()
{
    int slot = 1;
    ...
    clearmsg();
    print1("Current selected option is preceded by highlighted >>");
...
void bash_location()
{
    ...
    if(dir == ABORT) {
        ...
        if((Current_Environment == E_CITY) && (ox == 0) && (oy == 0)) {
            ...
            print2("You will invalidate your scroe if you proceed.");
            morewait();
            ...
            if(Level->site[ox][oy].locchar == WALL) {
                ...
                if((Player.patron > 0)
                   && (Level->site[ox][oy].aux == Player.patron)) {
                    ...
                    print1("The feeble powers of the minor gocling are not enough to");
                    ...
                    print2("Zzzap! A bolt of godsfire strikes!");

                    if(Player.rank[PRIESTHOOD] > 0) {
                        print3("You own deity's aegis defends you from the bolt!");
                    }
...
void save(int compress)
{
    ...
    if(ynq1() == 'y') {
        if(gamestatusp(ARENA_MODE)) {
            ...
            print3("Can't save the game in Adept's Challenge!");
            setgamestatus(SKIP_MONSTERS);
        }
        else {
            ...
            if(!compress) {
                print1("Warning: This file will not be compress.");
    ...
    if(p_moveable(Player.x + dx, Player.y + dy)) {
        if(Player.status[IMMOBILE] > 0) {
            ...
            if(gamestatusp(MOUNTED)) {
                ...
                print2("You steed bucks wildly and throws you off!");
            ...
            /*
             * Causes moves to take effectively 30 seconds in town
             * without monsters being sped up compared to player
             */
            ...
            /*
             * This test protects against player entering countryside
             * and still having effects from being on the Lelve, a
             * kludge, but hey...
             */
       ...
       && !Player.status[LEVITATING]) {
        ...
        if(p_country_moveable(Player.x + dx, Player.y + dy)) {
            if(Player.status[IMMOBILE] > 0) {
                   ...
                   && (Player.possessions[O_BOOTS] != NULL)) {
                    if(Player.possessions[O_BOOTS]->usef == I_BOOTS_7LEAGUE) {
                        ...
                        if(Player.possessions[O_BOOTS]->blessing < 0) {
                            print1("Whoah! -- Your boots launch you into the sky...");
                ...
                if(gamestatusp(LOST)
                   && (Precipitation < 1)
                   && Country[Player.x][Player.x].explored) {
