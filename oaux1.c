...
void showroom(int i)
{
    ...
    if((Current_Environment == E_MANSION) || (Current_Environment == E_HOUSE)) {
...
/*
 * Check a move attempt, maybe attack something, return TRUE if ok to move. x,y
 * is the proposed place to move to.
 */
...
void calc_melee()
{
    ...
    /*
     * Weapon 
     * Have to check for used since it could be a 2h weapon just carried in 
     * one hand
     */
...
void damage_item(pob o)
{
    ...
    if(o->id == (ARTIFACTID + 21)) {
        ...
        if(Current_Environment == E_STARSPEAK) {
            ...
            morewait();
            annihilate(1);
            print3("You seem to gain strenght in the chaotic glare of magic!");
        ...
        if(o->fragility < random_range(30)) {
            if(o->objchar == STICK) {
                ...
                if(o->blessing >= 0) {
                    strcat(Str1, o->truename);
                }
                else {
                    strcat(Str1, o->cursestr);
                }

                strcpy(Str1, " explodes!");
                print1(Str1);
                morewait();

                if(o->charge < 1) {
                    nprint1("Fzzz...Out of Power...Oh wll...");
                }
                else {
                    nprint1(" Ka-Blam!!!");

                    /* 
                     * General case. Some sticks will eventually do special
                     * things
                     */
                    morewait();
...
/* Bit wise access functions to Player.options */
int optionp(int o)
{
 ...
 * FALSE. Note if you're berserk you get to attack ALL adjacent monsters!
 */
int goberserk()
{
    int wentberserk = FALSE;
    int i;
    char meleestr[80];
