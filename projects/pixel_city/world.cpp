...
enum class fade_t { idle, out, wait, in };
...
static gl_rgba get_light_color(float sat, float lum) {
  return from_hsl(light_colors.at(random_val(light_colors.size())).hue, sat,
                  lum);
}
...
static void build_road(int x1, int y1, int width, int depth) {
  ...
  if (width > depth) {
    claim(x1, y1 + sidewalk, width, lanes,
          usage_t::claim_road | usage_t::map_road_west);
    claim(x1, y1 + sidewalk + lanes + divider, width, lanes,
          usage_t::claim_road | usage_t::map_road_west);
  } else {
    claim(x1 + sidewalk, y1, lanes, depth,
          usage_t::claim_road | usage_t::map_road_south);
    claim(x1 + sidewalk + lanes + divider, y1, lanes, depth,
          usage_t::claim_road | usage_t::map_road_south);
  }
}
...
void do_building(Plot p) {
  ...
  if (square && (p.width > 20)) {
    ...
    modern_count++;
    skyscrapers++;
    new Building(building_t::modern, p.x, p.z, height, p.width, p.depth, seed,
                 color);
...
static int build_light_strip(int x1, int z1, direction_t direction) {
  ...
  if (direction == direction_t::east) {
    d->create_light_strip(x1, z1 - size_adjust, width, depth + size_adjust, 2,
                          color);
  } else if (direction == direction_t::west) {
    d->create_light_strip(x1, z1, width, depth + size_adjust, 2, color);
  } else if (direction == direction_t::north) {
    d->create_light_strip(x1, z1, width + size_adjust, depth, 2, color);
  } else {
    d->create_light_strip(x1 - size_adjust, z1, width - size_adjust, depth, 2,
                          color);
  }
...
static void do_reset() {
  ...
  for (int y = WORLD_EDGE; y < (WORLD_SIZE - WORLD_EDGE);
       y += random_val(25) + 25) {
  ...
  broadway_done = false;
  for (int x = WORLD_EDGE; x < (WORLD_SIZE - WORLD_EDGE);
       x += random_val(25) + 25) {
  ...
  for (int x = 1; x < (WORLD_SIZE - 1); ++x) {
    for (int y = 0; y < WORLD_SIZE; ++y) {
      ...
      bool road_left =
          ((world.at(x + 1).at(y) & usage_t::claim_road) != usage_t::none);
      bool road_right =
          ((world.at(x - 1).at(y) & usage_t::claim_road) != usage_t::none);
      ...
      y += build_light_strip(
          x, y, road_right ? direction_t::south : direction_t::north);
    }
  }
  ...
  for (int y = 1; y < (WORLD_SIZE - 1); ++y) {
    for (int x = 1; x < (WORLD_SIZE - 1); ++x) {
      ...
      bool road_left =
          ((world.at(x).at(y + 1) & usage_t::claim_road) != usage_t::none);
      bool road_right =
          ((world.at(x).at(y - 1) & usage_t::claim_road) != usage_t::none);
      ...
      x += build_light_strip(
          x, y, road_right ? direction_t::east : direction_t::west);
    }
  }
  ...
  for (int x = 0; x < WORLD_SIZE; ++x) {
    for (int y = 0; y < WORLD_SIZE; ++y) {
      if (world.at(std::clamp(x, 0, WORLD_SIZE))
              .at(std::clamp(y, 0, WORLD_SIZE)) != usage_t::none) {
        continue;
      }
      ...
      while ((width > 8) && (depth > 8)) {
        if (!claimed(x, y, width, depth)) {
          ...
          if ((x < hot_zone.get_min().get_x()) ||
            ...
            new Building(building_t::simple, x + 1, y + 1, height, width - 2,
                         depth - 2, random_val(), building_color);
          } else {
            // Use fancy buildings.
            ...
            depth -= 2;
            if (random_val(2) == 0) {
              new Building(building_t::tower, x + 1, y + 1, height, width,
                           depth, random_val(), building_color);
            } else {
              new Building(building_t::blocky, x + 1, y + 1, height, width - 2,
                           depth - 2, random_val(), building_color);
            }
          }
...
gl_rgba world_light_color(unsigned int index) {
  ...
  return from_hsl(light_colors.at(index).hue, light_colors.at(index).sat,
...
gl_rgba world_bloom_color() { return bloom_color; }

int world_logo_index() { return logo_index++; }

gl_bbox world_hot_zone() { return hot_zone; }
...
float world_fade() { return fade_current; }

int world_scene_begin() { return scene_begin; }
...
void world_update() {
  ...
  if ((fade_state == fade_t::idle) &&
      (world_scene_elapsed() > RESET_INTERVAL.count())) {
    world_reset();
  }
}
