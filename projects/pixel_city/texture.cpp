...
static std::array<std::string, 23> prefix = {
    "i",      "Green ",    "Mega",   "Super ",    "Omni",      "e",
    "Hyper",  "GLobal ",   "Vital ", "Next ",     "Pacific ",  "Metro",
    "Unity ", "G-",        "Trans",  "Infinity ", "Superior ", "Monolith ",
    "Best ",  "Atlantic ", "First ", "Union ",    "National "};

std::array<std::string, 33> name = {
    "Biotic",      "Info",       "Data",    "Solar",     "Aerospace",
    "Motors",      "Nano",       "Online",  "Circuits",  "Energy",
    "Med",         "Robotic",    "Exports", "Security",  "Systems",
    "Financial",   "Industrial", "Media",   "Materials", "Foods",
    "Networks",    "Shipping",   "Tools",   "Medical",   "Publishing",
    "Enterprises", "Audio",      "Health",  "Bank",      "Imports",
    "Apparel",     "Petroleum",  "Studios"};

std::array<std::string, 15> suffix = {
    "Corp.", " Inc.", "Co",    "World", ".Com",       " USA", " Ltd.", "Net",
    " Tech", " Labs", " Mfg.", " UK",   " Unlimited", " One", " LLC"};

void Texture::clear() { ready_ = false; }
...
void drawrect_simple(int left, int top, int right, int bottom, gl_rgba color1,
                     gl_rgba color2) {
    ...
    if (bright) {
      ...
      for (int i = left + 1; i < right - 1; ++i) {
        for (int j = top + 1; j < bottom - 1; ++j) {
          ...
          float hue = 0.2f + (random_val(100) / 300.0f) +
                      (random_val(100) / 300.0f) + (random_val(100) / 300.0f);

          gl_rgba color_noise = from_hsl(hue, 0.3f, 0.5f);
          color_noise.set_alpha((random_val(potential) / 144.0f) * 255);
          glColor4f(random_val(256) / 256.0f, random_val(256) / 256.0f,
                    random_val(256) / 256.0f, random_val(potential) / 144.0f);
...
static void buildingWindow(int x, int y, int size, texture_t id,
                           gl_rgba color) {
  int i;
  ...
  case texture_t::building5:
    ...
    drawrect(x + size - margin - 1, y + 1, x + size - margin - 1, y + size - 1,
             color * 0.3f);
    break;
    ...
    for (int x = 0; x < SEGMENTS_PER_TEXTURE; ++x) {
      ...
      if (lit) {
        ...
        color = gl_rgba(127 + rand_val, 127 + rand_val, 127 + rand_val) +
                gl_rgba(random_val(10) / 50.0f, random_val(10) / 50.0f,
                        random_val(10) / 50.0f);
      } else {
      ...
      buildingWindow(x * segment_size_, y * segment_size_, segment_size_,
                     my_id_, color);
...
void Texture::draw_sky() {
  ...
  for (int i = 0; i < size_; i += 5) {
    drawrect(i, size_ - random_val(8) - random_val(8) - random_val(8),
             i + random_val(9), size_, gl_rgba(0, 0, 0));
  }
  ...
  for (int i = size_ - 30; i > 5; i -= 2) {
    ...
    for (int offset = -size_; offset <= size_; offset += size_) {
      for (float scale = 1.0f; scale > 0.0f; scale -= 0.25f) {
        ...
        width_adjust = ((width / 2.0f) + (inv_scale * (width / 2.0f)));
...
void Texture::rebuild() {
  ...
  // Set up the texutre
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size_, size_, 0, GL_RGBA,
  ...
  case texture_t::logos:
    ...
    while (i < size_) {
      ...
      if (random_val(2) == 0) {
        render_print(2, size_ - i - (LOGO_PIXELS / 4), random_val(),
                     gl_rgba(255, 255, 255), "%s%s",
                     prefix.at(prefix_num).c_str(), name.at(name_num).c_str());
      } else {
        render_print(2, size_ - i - (LOGO_PIXELS / 4), random_val(),
                     gl_rgba(255, 255, 255), "%s%s", name.at(name_num).c_str(),
  ...
  case texture_t::trim:
    ...
    for (int x = 0; x < size_; x += TRIM_PIXELS) {
      drawrect_simple(x + margin, y + margin, x + TRIM_PIXELS - margin,
                      y + TRIM_PIXELS - margin, gl_rgba(255, 255, 255),
                      gl_rgba(127, 127, 127));
    }
  ...
  if (mipmap_) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, size_, size_, GL_RGBA,
...
unsigned int texture_random_building(int index) {
  ...
  return texture_id(
      static_cast<texture_t>(static_cast<int>(texture_t::building1) + index));
}
...
bool texture_ready() { return textures_done; }
...
void texture_term() { textures.clear(); }
...
void texture_init() {
  ...
  for (int i = static_cast<int>(texture_t::building1);
       i < static_cast<int>(texture_t::building9); ++i) {
