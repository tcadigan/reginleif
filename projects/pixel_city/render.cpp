...
static std::chrono::milliseconds constexpr COLOR_CYCLE_TIME(
    10000); // Milliseconds
...
effect_t operator++(effect_t &effect, int val) {
  return static_cast<effect_t>((static_cast<int>(effect) + 1) %
                               static_cast<int>(effect_t::count));
}
...
static void do_progress(float center_x, float center_y, float radius,
...
static void do_effects(effect_t type) {
  ...
  case effect_t::color_cycle:
    ...
    hue1 = fmod(SDL_GetTicks(), COLOR_CYCLE_TIME.count()) /
           COLOR_CYCLE_TIME.count();

    offset = SDL_GetTicks() + COLOR_CYCLE;
    hue2 = fmod(offset, COLOR_CYCLE_TIME.count()) / COLOR_CYCLE_TIME.count();
    hue3 =
        fmod(offset * 2, COLOR_CYCLE_TIME.count()) / COLOR_CYCLE_TIME.count();
    hue4 =
        fmod(offset * 3, COLOR_CYCLE_TIME.count()) / COLOR_CYCLE_TIME.count();
  ...
  if (LOADING_SCREEN) {
    ...
    if (texture_ready() && !entity_ready() && (fade != 0.0f)) {
      ...
      do_progress(render_width / 2, render_height / 2, radius, fade,
                  entity_progress());

      render_print((render_width / 2) - LOGO_PIXELS,
                   (render_height / 2) + LOGO_PIXELS, 0, gl_rgba(127, 127, 127),
                   "%1.2f%%", entity_progress() * 100.0f);

      render_print(1, "%s v%d.%d.%03d", APP_TITLE.c_str(), VERSION_MAJOR,
...
void render_print(int x, int y, int font, gl_rgba color, const char *fmt, ...) {
  ...
  std::array<gl_font, 7> fonts = {
      gl_font{curier_new, 0},   gl_font{arial, 0},  gl_font{times_new_roman, 0},
      gl_font{arial_black, 0},  gl_font{impact, 0}, gl_font{agency_fb, 0},
...
void render_init() {
  ...
  if (!ini_int(render_section_ini, set_defaults_ini)) {
    ...
    ini_int_set(render_section_ini, effect_ini,
                static_cast<int>(effect_t::bloom));
...
bool render_fog() { return show_fog; }
...
bool render_wireframe() { return show_wireframe; }
...
bool render_bloom() {
  return ((effect == effect_t::bloom) || (effect == effect_t::bloom_radial) ||
          (effect == effect_t::debug_overbloom) ||
          (effect == effect_t::color_cycle));
}

bool render_flat() { return flat; }
...
void render_help_toggle() { show_help = !show_help; }

float render_fog_distance() { return fog_distance; }
...
void render_update() {
  ...
  // Framerate tracker
  if (show_fps) {
    render_print(1, "FPS=%d : Entities=%d : polys=%d", current_fps,
