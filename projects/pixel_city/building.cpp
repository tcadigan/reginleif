...
enum class addon_t { none = 0, logo, trim, lights, count };
 ...
 */
Building::Building(building_t type, int x, int y, int height, int width,
                   int depth, int seed, gl_rgba color) {
  x_ = x;
  y_ = y;
  width_ = width;
  height_ = height;
  center_ = gl_vector3((x_ + (width / 2.0f)), 0.0f, (y_ + (depth / 2.0f)));
  seed_ = seed;
  ...
  mesh_flat_ =
      std::make_unique<Mesh>(); // Flat-color mesh for untextured detail items
...
bool Building::alpha() const { return false; }
...
void Building::construct_cube(int left, int right, int front, int back,
                              int bottom, int top) {
...
void Building::construct_cube_float(float left, float right, float front,
                                    float back, float bottom, float top) {
  std::array<gl_vertex, 10> p;
  Cube cube;
 ...
 */
void Building::construct_roof(float left, float right, float front, float back,
                              float bottom) {
  ...
  if ((addon == addon_t::logo) && !have_logo_) {
    ...
    dec->create_light_trim(vector_buffer, 4, random_val(2) + 1.0f, seed_,
                           trim_color_);
  ...
  for (int i = 0; i < air_conditioners; ++i) {
    ...
    construct_cube(ac_x, ac_x + ac_size, ac_y, ac_y + ac_size, ac_base,
                   ac_base + ac_height);
  }

  if (height_ > 45) {
    Decoration *dec = new Decoration;
    dec->create_radio_tower(
        gl_vector3((left + right) / 2.0f, bottom, (front + back) / 2.0f),
        15.0f);
  }
}

void Building::construct_spike(int left, int right, int front, int back,
                               int bottom, int top) {
  gl_vertex p;
  Fan fan;
  gl_vector3 center;
 ...
 */
float Building::construct_wall(int start_x, int start_y, int start_z,
                               direction_t direction, int length, int height,
                               int window_groups, float uv_start,
                               bool blank_corners) {
  int step_x;
  int step_z;
  QuadStrip qs;
  ...
  for (int i = 0; i <= length; ++i) {
    ...
    if ((last_blank != blank) || (i == 0) || (i == length)) {
      ...
      v.get_uv().set_y((start_y + height) /
                       static_cast<float>(SEGMENTS_PER_TEXTURE));
      mesh_->vertex_add(v);
      qs.index_list.push_back(mesh_->vertex_count() - 1);
    }
...
void Building::create_blocky() {
  ...
  float uv_start = random_val(SEGMENTS_PER_TEXTURE) /
                   static_cast<float>(SEGMENTS_PER_TEXTURE);
  ...
  while (true) {
    ...
    if (!skip) {
      ...
      uv_start = construct_wall(mid_x - left, 0, mid_z + back,
                                direction_t::south, front + back, height,
                                grouping, uv_start, blank_corners) -
                 ONE_SEGMENT;
      uv_start = construct_wall(mid_x - left, 0, mid_z - front,
                                direction_t::east, right + left, height,
                                grouping, uv_start, blank_corners) -
                 ONE_SEGMENT;
      uv_start = construct_wall(mid_x + right, 0, mid_z - front,
                                direction_t::north, front + back, height,
                                grouping, uv_start, blank_corners) -
                 ONE_SEGMENT;
      uv_start = construct_wall(mid_x + right, 0, mid_z + back,
                                direction_t::west, right + left, height,
                                grouping, uv_start, blank_corners) -
                 ONE_SEGMENT;

      if (!tiers) {
        construct_roof((mid_x - left), (mid_x + right), (mid_z - front),
                       (mid_z + back), height);
      } else {
        // Add a flat-color lid onto this section
        construct_cube_float((mid_x - left), (mid_x + right), (mid_z - front),
                             (mid_z + back), height, height + lid_height);
      }
  ...
  construct_cube(mid_x - half_width, mid_x + half_width, mid_z - half_depth,
                 mid_z + half_depth, 0, 2);
...
void Building::create_simple() {
  ...
  float u = (random_val(SEGMENTS_PER_TEXTURE)) /
            static_cast<float>(SEGMENTS_PER_TEXTURE);
  float v1 = (random_val(SEGMENTS_PER_TEXTURE)) /
             static_cast<float>(SEGMENTS_PER_TEXTURE);
  ...
  mesh_->quad_strip_add(qs);
  construct_cube_float(x1 - ledge, x2 + ledge, z2 - ledge, z1 + ledge, height_,
                       height_ + cap_height);
  mesh_->compile();
}
...
void Building::create_modern() {
  ...
  for (int angle = 0; angle <= 360; angle += 10) {
    ...
    pos.set_x(center.get_x() -
              sinf(angle * DEGREES_TO_RADIANS) * radius.get_x());
    pos.set_z(center.get_z() +
              cosf(angle * DEGREES_TO_RADIANS) * radius.get_y());

    if ((angle > 0) && (skip_counter == 0)) {
      length = math_distance(p.get_position().get_x(), p.get_position().get_z(),
                             pos.get_x(), pos.get_z());
      ...
      if ((length > 10) && !logo_done) {
        ...
        end = gl_vector2(p.get_position().get_x(), p.get_position().get_z());

        d = new Decoration;
        gl_rgba random_color;
        d->create_logo(start, end, height_, world_logo_index(),
                       from_hsl(random_val(255) / 255.0f, 1.0f, 1.0f));
      }
  ...
  if (!logo_done && do_trim) {
    d = new Decoration;
    gl_rgba random_color;
    d->create_light_trim(vector_buffer, (points / 2) - 2, cap_height / 2.0f,
                         seed_, from_hsl(random_val(255) / 255.0f, 1.0f, 1.0f));
  }
...
void Building::create_tower() {
  ...
  construct_cube_float(left - ledge, right + ledge, front - ledge, back + ledge,
                       bottom, foundation);
  ...
  while (true) {
    ...
    uv_start = random_val(SEGMENTS_PER_TEXTURE) /
               static_cast<float>(SEGMENTS_PER_TEXTURE);

    uv_start =
        construct_wall(left, bottom, back, direction_t::south, section_depth,
                       section_height, grouping, uv_start, blank_corners) -
        ONE_SEGMENT;
    uv_start =
        construct_wall(left, bottom, front, direction_t::east, section_width,
                       section_height, grouping, uv_start, blank_corners) -
        ONE_SEGMENT;
    uv_start =
        construct_wall(right, bottom, front, direction_t::north, section_depth,
                       section_height, grouping, uv_start, blank_corners) -
        ONE_SEGMENT;
    uv_start =
        construct_wall(right, bottom, back, direction_t::west, section_width,
                       section_height, grouping, uv_start, blank_corners) -
        ONE_SEGMENT;
    ...
    construct_cube_float(left - ledge, right + ledge, front - ledge,
                         back + ledge, bottom, (bottom + ledge_height));
