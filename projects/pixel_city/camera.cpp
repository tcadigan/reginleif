...
camera_t operator++(camera_t &camera, int val) {
  return static_cast<camera_t>((static_cast<int>(camera) + 1) %
                               static_cast<int>(camera_t::modes));
}
...
static gl_vector3 flycam_position(unsigned int t) {
  ...
  switch (leg) {
  case 0:
    start = gl_vector3(hot_zone.get_min().get_x(), 25.0f,
                       hot_zone.get_min().get_z());

    end = gl_vector3(hot_zone.get_min().get_x(), 60.0f,
                     hot_zone.get_max().get_z());

    break;
  case 1:
    start = gl_vector3(hot_zone.get_min().get_x(), 60.0f,
                       hot_zone.get_max().get_z());

    end = gl_vector3(hot_zone.get_max().get_x(), 25.0f,
                     hot_zone.get_max().get_z());

    break;
  case 2:
    start = gl_vector3(hot_zone.get_max().get_x(), 25.0f,
                       hot_zone.get_max().get_z());

    end = gl_vector3(hot_zone.get_max().get_x(), 60.0f,
                     hot_zone.get_min().get_z());

    break;
  case 3:
    start = gl_vector3(hot_zone.get_max().get_x(), 60.0f,
                       hot_zone.get_min().get_z());

    end = gl_vector3(hot_zone.get_min().get_x(), 25.0f,
                     hot_zone.get_min().get_z());
...
static void do_auto_cam() {
  ...
  case camera_t::orbit_inward:
    auto_position.set_x(WORLD_HALF +
                        (sinf(tracker * DEGREES_TO_RADIANS) * 150.0f));

    auto_position.set_y(60.0f);

    auto_position.set_z(WORLD_HALF +
                        (cosf(tracker * DEGREES_TO_RADIANS) * 150.0f));
  ...
  case camera_t::orbit_outward:
    auto_position.set_x(WORLD_HALF +
                        (sinf(tracker * DEGREES_TO_RADIANS) * 250.0f));

    auto_position.set_y(60.0f);

    auto_position.set_z(WORLD_HALF +
                        (cosf(tracker * DEGREES_TO_RADIANS) * 250.0f));
  ...
  case camera_t::orbit_elliptical:
    ...
    auto_position.set_x(WORLD_HALF +
                        (sinf(tracker * DEGREES_TO_RADIANS) * dist));

    auto_position.set_y(60.0f);

    auto_position.set_z(WORLD_HALF +
                        (cosf(tracker * DEGREES_TO_RADIANS) * dist));
  ...
  default:
    ...
    auto_position.set_x(WORLD_HALF +
                        (sinf(tracker * DEGREES_TO_RADIANS) * 50.0f));

    auto_position.set_y(60.0f);

    auto_position.set_z(WORLD_HALF +
                        (cosf(tracker * DEGREES_TO_RADIANS) * 50.0f));
  }

  dist = math_distance(auto_position.get_x(), auto_position.get_z(),
                       target.get_x(), target.get_z());

  auto_angle.set_y(-math_angle(auto_position.get_x(), auto_position.get_y(),
                               target.get_x(), target.get_y()));

  auto_angle.set_x(90.f +
                   math_angle(0, auto_position.get_y(), dist, target.get_y()));
}

void camera_auto_toggle() { cam_auto = !cam_auto; }

void camera_next_behavior() { camera_behavior++; }

void camera_yaw(float delta) { angle.set_y(angle.get_y() - delta); }

void camera_pitch(float delta) { angle.set_x(angle.get_x() - delta); }
...
void camera_position_set(gl_vector3 new_pos) { position = new_pos; }
...
void camera_update() {
  ...
  angle.set_x(std::clamp(angle.get_x(), static_cast<float>(-MAX_PITCH),
                         static_cast<float>(MAX_PITCH)));
}
