...
public:
  ...
  void create_logo(gl_vector2 start, gl_vector2 end, float base, int seed,
                   gl_rgba color);

  void create_light_strip(float x, float z, float width, float depth,
                          float height, gl_rgba color);

  void create_light_trim(std::array<gl_vector3, MAX_VBUFFER> &chain, int count,
                         float height, int seed, gl_rgba color);
