...
static float constexpr LOGO_OFFSET =
    0.2f; // How far a logo sticks out from the given surface
...
bool Decoration::alpha() const { return use_alpha_; }

int Decoration::poly_count() const { return mesh_->poly_count(); }

unsigned Decoration::texture() const { return texture_; }
...
void Decoration::create_radio_tower(gl_vector3 pos, float height) {
  ...
  // Radio tower
  v.set_position(
      gl_vector3(center_.get_x(), center_.get_y() + height, center_.get_z()));

  v.set_uv(gl_vector2(0, 1));
  mesh_->vertex_add(v);

  v.set_position(gl_vector3(center_.get_x() - offset, center_.get_y(),
  ...
  std::shared_ptr<Light> light = std::make_shared<Light>(
      Light(gl_vector3(center_.get_x(), center_.get_y() + height + 1.0f,
                       center_.get_z()),
            gl_rgba(255, 192, 160), 1));
  light->blink();

  texture_ = texture_id(texture_t::lattice);
}

void Decoration::create_logo(gl_vector2 start, gl_vector2 end, float bottom,
  ...
  gl_vector3 out =
      cross_product(gl_vector3(0.0f, 1.0f, 0.0f), to) * LOGO_OFFSET;
...
void Decoration::create_light_strip(float x, float z, float width, float depth,
                                    float height, gl_rgba color) {
  gl_vertex p;
  QuadStrip qs1;
  float u;
  float v;
...
void Decoration::create_light_trim(std::array<gl_vector3, MAX_VBUFFER> &chain,
                                   int count, float height, int seed,
                                   gl_rgba color) {
  gl_vertex p;
  gl_vector3 to;
  gl_vector3 out;
