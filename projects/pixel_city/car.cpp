...
static std::array<gl_vector3, 4> direction = {
    gl_vector3(0.0f, 0.0f, -1.0f), gl_vector3(1.0f, 0.0f, 0.0f),
    gl_vector3(0.0f, 0.0f, 1.0f), gl_vector3(-1.0f, 0.0f, 0.0f)};
...
void Car::render() {
  ...
  angle = 360 - math_angle(position_.get_x(), position_.get_z(), pos.get_x(),
                           pos.get_z());
  ...
  glTexCoord2f(0, 0);
  glVertex3f(pos.get_x() + angles.at(angle).get_x(), -CAR_SIZE,
  ...
  glTexCoord2f(1, 1);
  glVertex3f(pos.get_x() - angles.at(angle).get_x(), top,
             pos.get_z() - angles.at(angle).get_y());
