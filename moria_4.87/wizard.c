...
void game_version()
{
    ...
    clear_screen(0, 0);
    sprintf(tmp_str, "               Moria Version %f", CUR_VERSION);
    ...
    put_buffer("Student/University of Oklahoma    Student/University of Oklahoma", 17, 0);
    ...
    put_buffer("similar to avoid confustion.", 10, 0);
    pause_line(23);
    draw_cave();
}
...
void change_character()
{
    ...
    if((tmp_val > -1) && (tmp_val < 32767)) {
        m_ptr->mana = tmp_val;
        m_ptr->cmana = (double)tmp_val;
        prt_cmana();
    }

    sprintf(tmp_str, "Current==%d  (0-200) Searching == ", m_ptr->srh);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    sscanf(tmp_str, "%d", &tmp_val);
    ...
    sprintf(tmp_str, "Current==%d  (0-10) Stealth == ", m_ptr->stl);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    ...
    sprintf(tmp_str, "Current==%d  (0-200) Disarming == ", m_ptr->disarm);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    ...
    sprintf(tmp_str, "Current==%d  (0-200) Save == ", m_ptr->save);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    ...
    sprintf(tmp_str, "Current==%d  (0-200) Base to hit == ", m_ptr->bth);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    ...
    sprintf(tmp_str, "Current=%d  (0-200) Bows/Throwing == ", m_ptr->bthb);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
    ...
    sprintf(tmp_str, "Current==%d  Gold == ", m_ptr->au);
    tmp_val = strlen(tmp_str);
    prt(tmp_str, 0, 0);
    get_string(tmp_str, 0, tmp_val, 10);
    tmp_val = -999;
...
void wizard_create()
{
    ...
    if(get_string(tmp_str, 0, 9, 80)) {
        strcpy(i_ptr->name, tmp_str);
    }
    else {
        strcpy(i_ptr->name, "& Wizard Object!");
    }
