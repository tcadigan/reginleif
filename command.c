...
void command(int tmode, char *f, ...)
{
    ...
    /* If they move, wakmonsters will mark them as awake */
    if(tmode != T_OTHER) {
        sleepmonster();
    }
...
void adjustpack(char *cmd)
{
    ...
    case 't':
        ...
        /* Don't blame weapon if arrow misses */
        hitstokill -= 1;
    ...
    case 'w':
        ...
        if(weaponclass(currentweapon) >= 100) {
            goodweapon = 1;
        }
        else {
            goodweapon = 0;
    ...
    case 'z':
        ...
        /* Don't blame weapon if wand misses */
        hitstokill -= 1;
...
/*
 * replaycommand: Find the old command int the log file and send it
 */
int replaycommand()
{
    char oldcmd[128];
